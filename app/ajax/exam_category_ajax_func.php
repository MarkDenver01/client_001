...
<?php 
    ...
    $sql = "SELECT * FROM exam_category WHERE exam_description='" .$exam_description. "'";
    $result = $db->query($sql);
    if ($result->num_rows > 0) {
        echo '<option selected>Select exam category</option>';
        while ($row = $result->fetch_assoc()) {
            echo '<option value="' .$row['exam_category']. '">' .$row['exam_category']. '</option>';
        }
    } else {
        echo '<option value="" style=">Exam description not available</option>';
    }
?>